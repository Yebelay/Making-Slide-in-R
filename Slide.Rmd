...
xaringanExtra::use_xaringan_extra(c("tile_view", "webcam", "editable", "panelset","scribble", "fit_screen", "share_again"))
...
- [General introduction to R markdown🐣](# General introduction to R markdown🐣)

- [Making presentation slide using xaringan template📝](#  Making presentation slide using xaringan template📝)

- [Create presentation using C4ED template 🤹](# Create presentation using C4ED template 🤹)
...
.pull-left[
```{r}
knitr::include_graphics("img/Ninja.png")
```
...
# Make slides look more interesting: <span style="color:orange">Adding colors</span>
...
.left[
+ `r fontawesome::fa("file-code")` the new Rmd file (i.e. `Xaringan Slide.Rmd`)

+ `r fontawesome::fa("folder-open")` the `img` folder containing logo files

+ `r fontawesome::fa("folder-open")` the `css` folder containing the custom `C4ED` theme files and an HTML file in charge of adding the c4ed logo to each slide.
]
] 
...
[c4ed_theme.css](C:\Users\YebelayBerehan\Making Slide in R\c4ed_theme.css)
...
```
class: inverse
background-image: url("https://media.giphy.com/media/giphy.gif")
...
class: inverse
background-image: url(https://media.giphy.com/media/cSiauzNn8OBLGq7Y1C/giphy.gif)
background-size: cover
